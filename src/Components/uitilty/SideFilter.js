...
import SidebarSearchHook from '../../Hoock/search/SidebarSearchHook';


export default function SideFilter() {
  const  [categore,brand,clickCategore,clickBrand,priceFrom,priceTo]=SidebarSearchHook();
      ...
      <Row>
        <div className="d-flex flex-column mt-2">
          ...
          <div className="d-flex mt-3">
            <input onChange={clickCategore} type="checkbox" value="0" />
            <div className="filter-sub me-2 ">الكل</div>
          </div>
          {
            categore ? (categore.map((item, index) => {
              return (
                <div key={index} className="d-flex mt-3">
                  <input onChange={clickCategore}  type="checkbox" value={item._id} />
